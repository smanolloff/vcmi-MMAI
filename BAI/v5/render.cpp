...
namespace MMAI::BAI::V5 {
    ...
    void Verify(const State* state) {
        ...
        auto ensureMeleeability = [=](BattleHex bh, HexAmoveMask mask, const CPlayerBattleCallback* battle, AMoveAction ama, const CStack* cstack, const char* flagname) {
            ...
            auto id = EI(ama) - EI(AMoveAction::MOVE_AND_ATTACK_0);
            auto estacks = state->battlefield->stacks->at(!EI(cstack->unitSide()));
            auto estack = estacks.at(id);
            auto canmelee = estack && CStack::isMeleeAttackPossible(cstack, estack->cstack, bh);
            auto canshoot = battle->battleCanShoot(cstack);

            // If AMOVE_N for hex H is allowed, then:
            // 1. Stack #N (enemy) must exist
            // 2. Hex H must be reachable
            // 3. Melee attacking N from H must be possible
            // 4. Our stack must NOT be able to shoot (custom restriction)
            ...
            if (estack && canreach && canmelee && !canshoot) {
                ASSERT(mvalue, msg);
            } else {
                ASSERT(!mvalue, msg);
            }
        ...
        auto ensureCorrectMaskOrNA = [=](BattleHex bh, int v, const CStack* cstack, const CPlayerBattleCallback* battle, const char* attrname) {
            ...
            auto mask = HexAmoveMask(v);

            ensureReachability(bh, mask.test(EI(AMoveAction::MOVE_ONLY)), cstack, (basename + "{MOVE_ONLY}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_0, cstack, (basename + "{MOVE_AND_ATTACK_0}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_1, cstack, (basename + "{MOVE_AND_ATTACK_1}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_2, cstack, (basename + "{MOVE_AND_ATTACK_2}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_3, cstack, (basename + "{MOVE_AND_ATTACK_3}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_4, cstack, (basename + "{MOVE_AND_ATTACK_4}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_5, cstack, (basename + "{MOVE_AND_ATTACK_5}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_6, cstack, (basename + "{MOVE_AND_ATTACK_6}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_7, cstack, (basename + "{MOVE_AND_ATTACK_7}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_8, cstack, (basename + "{MOVE_AND_ATTACK_8}").c_str());
            ensureMeleeability(bh, mask, battle, AMoveAction::MOVE_AND_ATTACK_9, cstack, (basename + "{MOVE_AND_ATTACK_9}").c_str());
        };
        ...
        for (int ihex=0; ihex < BF_SIZE; ihex++) {
            ...
            for (int i=0; i < EI(HexAttribute::_count); i++) {
                ...
                break; case HA::STACK_ID: {
                    ...
                    if (slot >= 0) {
                        // regular stack -- STACK_ID is 0..6
                        expect(v == slot, "HEX.STACK_ID: =%d, but cstack->unitSlot()=%d", v, slot);
                        break;
                    }
                    ...
                    if (extraSeqNumber < MAX_STACKS_PER_SIDE - 7) {
                        ...
                        expect(v == idForSide, "HEX.STACK_ID: v=%d, but expected %d (extraSeqNumber=%d)", v, idForSide, extraSeqNumber);
                        break;
                    }
                    ...
                    if (nFreeRegularIds >= nFreeRegularIdsNeeded) {
                        ...
                        expect(v < 7,
                            "HEX.STACK_ID: v=%d, but expected v < 7 (extraSeqNumber=%d, nFreeRegularIds=%d)",
                            v, extraSeqNumber, nFreeRegularIds
                        );

                        // the ID our extra stack uses must NOT be used by a regular stack
                        auto cstackAtID = regulars.at(v);
                        expect(cstackAtID == nullptr, "HEX.STACK_ID: v=%d, but there's a stack at this regular id", v);
                ...
                break; case HA::STACK_SIDE:
                    if (cstack == nullptr) {
                        expect(v == NULL_VALUE_UNENCODED, "HEX.STACK_SIDE: v=%d, but cstack is nullptr", v);
                        break;
                    }
                    expect(v == EI(cstack->unitSide()), "HEX.STACK_SIDE: =%d, but cstack->unitSide() is %d", v, EI(cstack->unitSide()));
                ...
                for (int i=0; i < EI(SA::_count); i++) {
                    ...
                    break; case SA::FLAGS:
                        for (int j=0; j<EI(StackFlag::_count); j++) {
                            auto f = StackFlag(j);
                            auto vf = stack->flag(f);

                            switch(f) {
                            break; case SF::CAN_WAIT:
                                ensureValueMatch(vf, cstack->willMove() && !cstack->waitedThisTurn, "STACK.FLAGS.CAN_WAIT");
                            break; case SF::WILL_ACT:
                                ensureValueMatch(vf, cstack->willMove(), "STACK.FLAGS.WILL_ACT");
                            break; case SF::CAN_RETALIATE:
                                ...
                                ensureValueMatch(vf, cstack->ableToRetaliate(), "STACK.FLAGS.CAN_RETALIATE");
                            break; case SF::SLEEPING:
                                cstack->unitType()->getId() == CreatureID::AMMO_CART
                                    ? ensureValueMatch(vf, false, "STACK.FLAGS.SLEEPING")
                                    : ensureValueMatch(vf, cstack->hasBonusOfType(BonusType::NOT_ACTIVE), "STACK.FLAGS.SLEEPING");
                            break; case SF::BLOCKED:
                                ensureValueMatch(vf, cstack->canShoot() && battle->battleIsUnitBlocked(cstack), "STACK.FLAGS.TWO_HEX_ATTACK_BREATH");
                            break; case SF::BLOCKING:
                                ...
                                ensureValueMatch(vf, bool(want), "STACK.FLAGS.BLOCKING");
                            break; case SF::IS_WIDE:
                                ensureValueMatch(vf, cstack->occupiedHex().isAvailable(), "STACK.FLAGS.IS_WIDE");
                            break; case SF::FLYING:
                                ensureValueMatch(vf, cstack->hasBonusOfType(BonusType::FLYING), "STACK.FLAGS.FLYING");
                            break; case SF::BLIND_LIKE_ATTACK:
                                ensureValueMatch(vf, castchance({SpellID::BLIND, SpellID::STONE_GAZE, SpellID::PARALYZE}) > 0, "STACK.FLAGS.BLIND_LIKE_ATTACK");
                            break; case SF::ADDITIONAL_ATTACK:
                                ensureValueMatch(vf, cstack->hasBonusOfType(BonusType::ADDITIONAL_ATTACK), "STACK.FLAGS.ADDITIONAL_ATTACK");
                            break; case SF::NO_MELEE_PENALTY:
                                ensureValueMatch(vf, cstack->hasBonusOfType(BonusType::NO_MELEE_PENALTY), "STACK.FLAGS.NO_MELEE_PENALTY");
                            break; case SF::TWO_HEX_ATTACK_BREATH:
                                ensureValueMatch(vf, cstack->hasBonusOfType(BonusType::TWO_HEX_ATTACK_BREATH), "STACK.FLAGS.TWO_HEX_ATTACK_BREATH");
                            break; case SF::BLOCKS_RETALIATION:
                                ensureValueMatch(vf, cstack->hasBonusOfType(BonusType::BLOCKS_RETALIATION), "STACK.FLAGS.BLOCKS_RETALIATION");
                            break; default:
        ...
        for (int i=0; i<EI(PrimaryAction::_count); i++) {
            auto pa = PrimaryAction(i);
            auto v = state->actmask.at(i);

            switch(pa) {
            break; case Schema::V5::PrimaryAction::RETREAT:
                expect(v, "mask[RETREAT] must be always allowed");
            break; case Schema::V5::PrimaryAction::WAIT:
                expect(v != astack->waitedThisTurn, "mask[WAIT]=%d, but waitedThisTurn=%d", v, astack->waitedThisTurn);
            break; case Schema::V5::PrimaryAction::MOVE:
                expect(v, "mask[MOVE] must be always allowed");
            break; case Schema::V5::PrimaryAction::ATTACK_0:
                   case Schema::V5::PrimaryAction::ATTACK_1:
                   case Schema::V5::PrimaryAction::ATTACK_2:
                   case Schema::V5::PrimaryAction::ATTACK_3:
                   case Schema::V5::PrimaryAction::ATTACK_4:
                   case Schema::V5::PrimaryAction::ATTACK_5:
                   case Schema::V5::PrimaryAction::ATTACK_6:
                   case Schema::V5::PrimaryAction::ATTACK_7:
                   case Schema::V5::PrimaryAction::ATTACK_8:
                   case Schema::V5::PrimaryAction::ATTACK_9:
                for (int ihex=0; ihex < BF_SIZE; ihex++) {
                    int x = ihex%15;
                    int y = ihex/15;
                    auto &hex = state->battlefield->hexes->at(y).at(x);
                    auto mask = HexAmoveMask(hex->attr(HA::ACTION_MASK));
                    auto ama = Hex::PrimaryToAMoveAction(pa);
                    auto stackind = EI(pa) - EI(Schema::V5::PrimaryAction::ATTACK_0);
                    ensureMeleeability(hex->bhex, mask, battle, ama, astack, ("mask[MOVE_AND_ATTACK_" + std::to_string(stackind) + "]").c_str());
                }
            break; default:
                throw std::runtime_error("Unexpected PrimaryAction: " + std::to_string(EI(pa)));
            }
        }

        // Move is always allowed (because of DEFEND)
        expect(state->actmask.at(EI(PrimaryAction::MOVE)), "Mask does not allow MOVE (defending must be always allowed)");
    }
    ...
    std::string Render(const Schema::IState* istate, const Action *action) {
        ...
        auto idstacks = std::array<std::array<IStack*, MAX_STACKS_PER_SIDE>, 2>{};

        // find an active hex (i.e. with active stack on it)
        for (auto &sidestacks : allstacks) {
            for (auto &stack : sidestacks) {
                if (stack) {
                    idstacks.at(stack->getAttr(SA::SIDE)).at(stack->getAttr(SA::ID)) = stack;
        ...
        static_assert(MAX_STACKS_PER_SIDE == 10, "code assumes 10 stacks per side");
        ...
        auto seenstacks = std::array<std::bitset<MAX_STACKS_PER_SIDE>, 2>{0, 0};
        bool divlines = true;
        ...
        for (int y=0; y < BF_YMAX; y++) {
            for (int x=0; x < BF_XMAX; x++) {
                ...
                if (hex->getAttr(HA::STACK_ID) != NULL_VALUE_UNENCODED) {
                    stack = idstacks.at(hex->getAttr(HA::STACK_SIDE)).at(hex->getAttr(HA::STACK_ID));
                    expect(stack, "stack with side=%d and ID=%d not found", hex->getAttr(HA::STACK_SIDE), hex->getAttr(HA::STACK_ID));
                }
                ...
                auto amask = HexAmoveMask(hex->getAttr(HA::ACTION_MASK));
                if (col == nocol && !amask.test(EI(AMoveAction::MOVE_ONLY))) { // || supdata->getIsBattleEnded()
                    col = darkcol;
                    sym = sym == "○" ? "◌" : sym;
                }

                if (hex->getAttr(HA::STACK_ID) >= 0) {
                    auto seen = seenstacks.at(stack->getAttr(SA::SIDE)).test(stack->getAttr(SA::ID));
                    auto &[_, _e, n, _vmax] = STACK_ENCODING.at(EI(SA::FLAGS));
                    ...
                    seenstacks.at(stack->getAttr(SA::SIDE)).set(stack->getAttr(SA::ID));
                }
        ...
        const auto rowdefs = std::vector<RowDef> {
            ...
            RowDef{SA::Y_COORD, "State"},  // "WAR" = CAN_WAIT, WILL_ACT, CAN_RETAL
            RowDef{SA::Y_COORD, "Attack mods"}, // "DB" = Double, Blinding
...
// Y_COORD, "Wide/Breath"},
                            ...
                            break; case 2: value = boost::str(fmt % flags.test(EI(SF::BLOCKED)) % flags.test(EI(SF::BLOCKING)));
                            break; case 3: value = boost::str(fmt % flags.test(EI(SF::FLYING)) % flags.test(EI(SF::SLEEPING)));
                            break; case 4: value = boost::str(fmt % flags.test(EI(SF::BLOCKS_RETALIATION)) % flags.test(EI(SF::NO_MELEE_PENALTY)));
                            break; case 5: value = boost::str(fmt % flags.test(EI(SF::IS_WIDE)) % flags.test(EI(SF::TWO_HEX_ATTACK_BREATH)));
                            break; default:
