...
namespace MMAI::BAI::V5 {
    using namespace Schema::V5;
    // MOVE(1) + MELEE(10)

    using HexAmoveMask = std::bitset<EI(AMoveAction::_count)>;
     ...
     */
    class Hex : public Schema::V5::IHex {
    public:
        static PrimaryAction AMoveToPrimaryAction(AMoveAction ama);
        static AMoveAction PrimaryToAMoveAction(PrimaryAction pa);
        static int CalcId(const BattleHex &bh);
        static std::pair<int, int> CalcXY(const BattleHex &bh);

        Hex(const BattleHex &bh); // for unavailable hexes
        Hex(
            const BattleHex &bh,
        ...
        HexAttrs attrs = {};
        HexAmoveMask amovemask = 0;    // for active stack only
