...
#include "BAI/v11/action.h"
#include "BAI/v11/hexaction.h"
    ...
    Schema::Action BAI::getNonRenderAction() {
        ...
        while (action == Schema::ACTION_RENDER_ANSI) {
            if (state->supdata->ansiRender.empty()) {
                ...
                state->supdata->type = Schema::V11::ISupplementaryData::Type::ANSI_RENDER;
            }
        ...
        state->supdata->type = Schema::V11::ISupplementaryData::Type::REGULAR;
        return action;
    }
            ...
            case HexAction::MOVE: {
                ...
                if (a == EAccessibility::OBSTACLE) {
                    ...
                    if (bh.toInt() == acstack->getPosition().toInt()) {
                        ...
                        ASSERT(rinfo.distances.at(bh.toInt()) == ReachabilityInfo::INFINITE_DIST, "mask prevented (A)MOVE to self-occupied hex" + debugInfo(action, acstack, nullptr));
